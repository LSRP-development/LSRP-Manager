import { ActionRowBuilder, ButtonBuilder, ButtonStyle, Client, Interaction, MessageReferenceType, Snowflake } from "discord.js";
...
export default async function (interaction: Interaction, client: Client<true>) {
  ...
  const [, vote, id]: [unknown, "up" | "down", string] = interaction.customId.split("_") as ["sug", "up" | "down", string];

  const document = await Suggestions.findOne({ ID: id });
  if (!document) {
    await interaction.followUp({ flags: "Ephemeral", content: "Suggestion not found" });
    return;
  }

  const suggestionsChannel = await client.channels.fetch(mainConfigManager.config.channels.suggestions);
  if (!suggestionsChannel || !suggestionsChannel.isSendable() || !suggestionsChannel.isTextBased()) return;

  await suggestionsChannel.messages.fetch();
  const suggestionMessage = suggestionsChannel.messages.cache.get(document.messageID);
  ...
  const [upvotes, downvotes] = _.partition([...votesMap.values()], v => v === "up");
  ...
  await document.updateOne({ $set: { votes: [...votesMap.entries()].length > 0 ? votesMap : null } });

  if (count.up >= mainConfigManager.config.suggestionUpvoteTreshold) {
    const mgmtChannel = await client.channels.fetch(mainConfigManager.config.channels.mgmtSuggestionNotifications);
    ...
    if (messages.find(msg => {
      ...
      if (!reference || msg.reference?.type !== MessageReferenceType.Forward) return false;
      return suggestionsChannel.messages.cache.get(reference)?.embeds.at(0)?.footer?.text.includes(`ID: ${document.ID}`);
    })) return;
