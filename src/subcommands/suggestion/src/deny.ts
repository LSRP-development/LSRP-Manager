...
export default async function ({ interaction }: SlashCommandProps) {
  ...
  await interaction.editReply({
    embeds: [getCommandLoadingEmbed("Getting suggestion info...")],
  });
  ...
  if (!document) {
    await interaction.editReply({
      embeds: [getCommandFailedToRunEmbed("Suggestion not found")],
    });
    return;
  }
  ...
  if (document.approvalStatus) {
    await interaction.editReply({
      embeds: [
        getCommandFailedToRunEmbed("Suggestion is already approved/denied"),
      ],
    });
    return;
  }

  const suggestionsChannel = await interaction.client.channels.fetch(
    mainConfigManager.config.channels.suggestions,
  );
  if (!suggestionsChannel || !suggestionsChannel.isTextBased()) return;

  const suggestionMessage = await suggestionsChannel.messages.fetch(
    document.messageID,
  );
  if (!suggestionMessage) {
    await interaction.editReply({
      embeds: [getCommandFailedToRunEmbed("Suggestion message not found")],
    });
    return;
  }

  await interaction.editReply({
    embeds: [getCommandLoadingEmbed("Denying suggestion...")],
  });
  ...
  const [upvotes, downvotes] = _.partition(
    [...(document.votes ?? new Map<Snowflake, "up" | "down">()).values()],
    (v) => v === "up",
  );
  ...
  const embed = new EmbedBuilder()
    .setTitle("Suggestion")
    .setDescription(`**Denied** - ${reason}`)
    .addFields([
      {
        name: "Submitted by:",
        value: `<@!${document.author}> (\`${document.author}\`)`,
      },
      {
        name: "Suggestion:",
    ...
    .setFooter({
      text: `Denied by ${interaction.user.username} | ID: ${document.ID}`,
  ...
  await suggestionMessage
    .edit({ components: [actionRow], embeds: [embed] })
    .catch(() => null);
